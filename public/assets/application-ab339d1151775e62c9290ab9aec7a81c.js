...
(function() {
  $(function() {
    $("a.deleteEntry[data-remote]").on('mousedown', function(event) {
      console.log("mousedown");
      return $(this).parent().append("deleting...");
    });
    return $("a.deleteEntry[data-remote]").on("ajax:success", function(e, data, status, xhr) {
      console.log("ajax success");
      $(this).parent().hide();
      return $(this).parent().append("deleted!");
    });
  });
...
(function() {
  $(function() {
    $("a.deleteEntry[data-remote].remove").on('mousedown', function(event) {
      console.log("mousedown");
      return $(this).parent().append("deleting...");
    });
    $("a.deleteEntry[data-remote]").on("ajax:success", function(e, data, status, xhr) {
      console.log("ajax success");
      $(this).parent().hide();
      return $(this).parent().append("deleted!");
    });
    $("a.addEntry[data-remote]").on('mousedown', function(event) {
      return console.log('mousedown');
    });
    return $("a.addEntry[data-remote]").on("ajax:success", function(e, data, status, xhr) {});
  });
...
(function() {
  $(function() {
    $("#new_entry").on("ajax:success", function(e, data, status, xhr) {
      return $("#new_entry").append("<p>posted! </p>");
    }).on("ajax:error", function(e, xhr, status, error) {
      return $("#new_entry").append("<p>ERROR</p>");
    });
    return $("#new_entry").on('submit', function(event) {
      return $(this).append("<p>posting... </p>");
    });
  });
...
(function() {
  $(function() {
    var addHandler, appendControllerData, appendEntryHTML, appendUserHTML, appendUsers, buildAttachedHTML, buildHandler, buildRequestHTML, buildUserHTML, dbSearchThenAppend, followHandler, loaded, loading, searchEntries, searchUsers, suggestHandler, zapHandler;
    ...
    loaded = function(model) {
      $('.pagination').hide();
      return $('.loading-' + model).hide();
    };
    dbSearchThenAppend = function(url, list, type) {
      if (((url !== 'search/users/') && (url !== '/null')) && (url !== 'search/entries/') && (url !== 'search/entries//no') && (url !== '/null')) {
        loading(type);
        return $.ajax(url, {
          type: 'GET',
          error: function(jqXHR, textStatus, errorThrown) {},
          success: function(data) {
            return appendControllerData(data, type, list);
          }
        });
      }
    };
    searchUsers = function(keycode, cssClass) {
      var entries, entry, list, searchValue, type, url, _i, _len;
      type = "users";
      searchValue = $(cssClass).children().first().val();
      url = "/search/" + type + "/" + searchValue + "/no";
      if (cssClass === '.search-entries') {
        list = $('#entries-list');
      } else {
        list = $('#users-list');
      }
      ...
      loaded(type);
      return dbSearchThenAppend(url, list, type);
    };
    searchEntries = function(keycode) {
      var entries, entry, list, pageLimit, searchValue, type, url, _i, _len;
      type = "entries";
      ...
      if (keycode === 13 && type === 'entries') {
        ...
        url = "http://api.rottentomatoes.com/api/public/v1.0/movies.json?apikey=3gh3a35gm2x9x3as2qp69rr4&q=";
    ...
    buildUserHTML = function(amazonPath, user_id, name, followers) {
      var add, html, isZap, zap;
      if (window.location.pathname === '/search') {
    ...
    buildRequestHTML = function(requestData, picture, title, rt_id) {
      var add, html, suggestion;
      suggestion = false;
      if ($('.suggest').length > 0) {
        suggestion = true;
      }
    ...
    buildAttachedHTML = function(picture, title, id) {
      var add, html, suggestion;
      html = "<li class='new_well '>";
    ...
    appendUserHTML = function(data) {
      var add, amazonId, amazonPath, html, unfollow;
      amazonPath = "http://s3.amazonaws.com/zappcord/users/avatars/000/000/";
      amazonId = "00" + data['id'];
      if (amazonId.length === 4) {
        amazonId = "0" + data['id'];
      } else if (amazonId.length === 5) {
        amazonId = data['id'];
      }
    ...
    appendEntryHTML = function(data) {
      var del, html, photo, title;
    ...
    suggestHandler = function(event) {
      var title;
      event.preventDefault();
      title = $(this).parent().children()[1].innerHTML;
      $('#zap_title').val(title);
      return $('#entry_id').val($(this).children()[0].id);
    };
    addHandler = function(event) {
      ...
      $.get(url, {
        ...
        return appendEntryHTML(data);
      });
      return $.get('/search/reindex');
    };
    buildHandler = function(event) {
      var clickedElement, suggestion, url, values;
      suggestion = false;
      if ($('.suggest').length > 0) {
        suggestion = true;
      }
      ...
      return $.get(url, {
        ...
        if (clickedElement.className.substring(0, 7) === 'unbuilt') {
          return window.location.replace("/entries/" + data['id']);
    ...
    followHandler = function(event) {
      ...
      return $.get(url, {
        ...
        return appendUserHTML(data);
      });
    };
    appendUsers = function(data, list, followers) {
      var add, amazonId, amazonPath, html, k, max, name, v, _results;
      max = 5;
      _results = [];
      for (k in data) {
        v = data[k];
        ...
        if (followers) {
          ...
          list.append(html);
          add = $('.zapp' + v['id']);
          _results.push(add.click(zapHandler));
        } else {
          ...
          if ($('.zap').length > 0) {
            ...
            _results.push(add.click(followHandler));
          }
        }
      }
      return _results;
    };
    appendControllerData = function(data, type, list) {
      var addAnchor, html, k, max, picture, suggestAnchor, v, _results;
      loaded(type);
      max = 5;
      if (type === 'entries') {
        _results = [];
        ...
        return appendUsers(data, list, true);
      } else {
        return appendUsers(data, list, false);
      }
    };
    window.appendRequestData = function(data) {
      var add, alternate, html, imdb, list, movie, requestData, show, _i, _len, _ref, _results;
      loaded('entries');
      list = $('#entries-list');
      _ref = data["movies"];
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        _results.push(add.click(buildHandler));
      }
      return _results;
    };
    $('.depth').on('click', function(event) {
    ...
    $('.search-users').on('keypress', function(event) {
      var keycode;
      keycode = event.keyCode;
      return searchUsers(keycode, '.search-users');
    });
    return $('.search-entries').on('keypress', function(event) {
      var keycode, pathName, type;
      keycode = event.keyCode;
      pathName = window.location.pathname;
      if (pathName === "/search") {
        type = $('input:checked').val();
        if (type === "entries") {
          return searchEntries(keycode);
        } else {
          return searchUsers(keycode, ".search-entries");
        }
      } else {
        return searchEntries(keycode);
      }
...
(function() {
  $(function() {
    return $("a#unfollow[data-remote]").on("ajax:success", function(e, data, status, xhr) {
      return $(this).parent().hide();
    });
  });
...
(function() {
  $(function() {
    return $('#submit_zap').on('click', function(event) {
      var entry_id, receiver_id;
      receiver_id = $('#receiver_id').val();
      entry_id = $('#entry_id').val();
      console.log(entry_id);
      console.log(receiver_id);
      if (receiver_id === "" || entry_id === "" || receiver_id === null || entry_id === null) {
        event.preventDefault();
        return alert('Please select a movie and friend');
      }
    });
  });
